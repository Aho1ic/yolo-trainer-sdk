...
class C3k(C3):
    """C3 module with configurable bottleneck kernel size, used by YOLO11."""
...
class C3k2(C2f):
    """Faster CSP bottleneck with optional C3k blocks, used by YOLO11."""
...
class PSABlock(nn.Module):
    """Position-sensitive attention block used by YOLO11 C2PSA/C2fPSA."""
    ...
    def forward(self, x):
        """Forward pass through attention and feed-forward layers."""
...
class C2PSA(nn.Module):
    """C2 module with stacked PSA blocks, used by YOLO11."""
    ...
    def forward(self, x):
        """Forward pass through stacked PSA blocks."""
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = self.m(b)
        return self.cv2(torch.cat((a, b), 1))
